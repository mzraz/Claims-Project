...
import { getEmployeesReport } from '../../../store/report/ReportSlice';
...
const EmployeeReportList = () => {
    ...
    const getEmployeesList = useSelector((state) => state.reportReducer.employeeList);

    const [rows, setRows] = React.useState(getEmployeesList);
    const [search, setSearch] = React.useState('');

    React.useEffect(() => {
        setRows(getEmployeesList);
    }, [getEmployeesList]);

    const handleSearch = (event) => {
        const filteredRows = getEmployeesList.filter((row) => {
